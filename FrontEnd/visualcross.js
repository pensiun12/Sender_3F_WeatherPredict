document.addEventListener("DOMContentLoaded", () => {
    ...
    // Logika ini hanya akan berjalan jika elemen dari setting.html ditemukan.
    ...
    // Anda bisa menambahkan selektor untuk #themeSelect dan #logoutOption di sini jika ingin menangani logikanya di visualcross.js
    ...
    if (cityInputElement && temperatureElement && displayedLocationNameSpan) {
        ...
        // --- Fungsi untuk Mendapatkan Preferensi dan Simbol Unit Suhu (BARU/DIMODIFIKASI) ---
        function getTemperatureUnitPreference() {
            return localStorage.getItem(TEMP_UNIT_KEY) || 'metric';
        }

        function getTemperatureSymbol(unitPreference) {
            return unitPreference === 'us' ? '°F' : '°C';
        }
        // --- Akhir Fungsi Preferensi Suhu ---
        ...
        if (favoriteLocationIcon && displayedLocationNameSpan) { favoriteLocationIcon.addEventListener('click', () => { const currentLocationName = displayedLocationNameSpan.textContent.trim(); if (!currentLocationName || currentLocationName === "MEMUAT LOKASI..." || currentLocationName === "LOKASI TIDAK DIKETAHUI" || currentLocationName === "N/A") { console.warn("Tidak bisa memfavoritkan lokasi yang belum valid atau diketahui:", currentLocationName); return; } if (isLocationFavorite(currentLocationName)) { removeLocationFromFavorites(currentLocationName); } else { addLocationToFavorites(currentLocationName); } updateFavoriteIconState(currentLocationName); });
        } else { /* console.warn("Elemen untuk ikon favorit (#favoriteLocationIcon) atau nama lokasi (#displayedLocationNameSpan) tidak ditemukan di DOM."); */ }
        ...
        // --- displayCurrentWeather dengan Penyesuaian Unit Suhu ---
        function displayCurrentWeather(currentConditions, resolvedAddress, timezoneOffsetSeconds) {
            ...
            if (weatherIconElement) { weatherIconElement.src = localIconPath; weatherIconElement.alt = indonesianDescription; }
            ...
            let windSpeedToDisplay = Math.round(currentConditions.windspeed);
            let windUnit = "KM/J";
            ...
            if (sunTimeElement) { sunTimeElement.style.opacity = "1"; sunTimeElement.style.visibility = "visible"; }
        ...
        // --- displayHourlyForecast dengan Penyesuaian Unit Suhu ---
        function displayHourlyForecast(hoursData, timezoneOffsetSeconds) {
            const tempUnitPref = getTemperatureUnitPreference();
            const tempSymbol = getTemperatureSymbol(tempUnitPref);
            if (!hoursData || !hourlySliderElement) { if (hourlySliderElement) hourlySliderElement.innerHTML = "<p style='text-align:center; width:100%;'>Data prakiraan per jam tidak tersedia.</p>"; return; }
            hourlySliderElement.innerHTML = "";
            const displayHoursCount = 24;
            hoursData.slice(0, displayHoursCount).forEach(hour => {
                if (typeof hour.icon !== 'string') { console.warn("Data ikon jam tidak valid:", hour); return; }
                const card = document.createElement("div"); card.classList.add("hour-card");
        ...
        // --- displayDailyForecast dengan Penyesuaian Unit Suhu ---
        function displayDailyForecast(daysData, timezoneOffsetSeconds) {
            ...
            if (!daysData || !dailyForecastElement) { if (dailyForecastElement) dailyForecastElement.innerHTML = "<p style='text-align:center; width:100%;'>Data prakiraan harian tidak tersedia.</p>"; return; }
            dailyForecastElement.innerHTML = "";
            daysData.slice(0, 7).forEach(day => {
                if (typeof day.icon !== 'string') { console.warn("Data ikon harian tidak valid:", day); return; }
                const card = document.createElement("div"); card.classList.add("daily-card");
                ...
                let windSpeedToDisplay = Math.round(day.windspeed);
                let windUnit = "km/j";
        ...
        // --- getWeatherData dengan Penyesuaian Unit Suhu untuk API Call ---
        async function getWeatherData(cityName) {
            const cityToFetch = cityName;
            const currentSelectedUnit = getTemperatureUnitPreference(); // DAPATKAN PREFERENSI UNIT
            ...
            if (displayedLocationNameSpan) displayedLocationNameSpan.textContent = "MEMUAT LOKASI...";
            updateFavoriteIconState(null);
            if (sunTimeElement) { sunTimeElement.style.opacity = "0"; sunTimeElement.style.visibility = "hidden"; }

            const apiUrl = `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${encodeURIComponent(cityToFetch)}/next7days?key=${VC_API_KEY}&contentType=json&unitGroup=${currentSelectedUnit}&lang=id&include=hours,current,days`;
            // console.log(`[VC API] Fetching for unitGroup: ${currentSelectedUnit}`);

            try {
                const response = await fetch(apiUrl);
                if (!response.ok) { let errorMsg = response.statusText; try { const errorData = await response.json(); errorMsg = errorData.message || JSON.stringify(errorData) || response.statusText; } catch (e) {} throw new Error(`Gagal mengambil data dari Visual Crossing: ${response.status} - ${errorMsg}`); }
                ...
                } else { if(hourlySliderElement) hourlySliderElement.innerHTML = "<p style='text-align:center; width:100%;'>Data per jam tidak tersedia untuk hari ini.</p>" }
                if (data.days && Array.isArray(data.days)) {
                    displayDailyForecast(data.days, timezoneOffsetSeconds);
                } else { if(dailyForecastElement) dailyForecastElement.innerHTML = "<p style='text-align:center; width:100%;'>Data prakiraan harian tidak tersedia.</p>" }
                ...
                if (displayedLocationNameSpan) displayedLocationNameSpan.textContent = "N/A";
                updateFavoriteIconState(null);
                globalSunriseEpoch = undefined; globalSunsetEpoch = undefined; globalTimezoneOffsetSeconds = undefined; 
                if (sunRealTimeUpdateInterval) clearInterval(sunRealTimeUpdateInterval); 
                if(sunTimeElement){sunTimeElement.style.opacity = "0"; sunTimeElement.style.visibility = "hidden";}
            }
        }
        ...
        // Event Listeners untuk search (homepage)
    ...
    // Slider Logic & Hamburger Menu (Tidak berubah dari kode Anda, KECUALI jika selector hamburger perlu disesuaikan)
    let currentPositionHourly = 0; const btnLeftHourly = document.querySelector(".hourly-forecast .arrow-left"); const btnRightHourly = document.querySelector(".hourly-forecast .arrow-right"); const getCardWidthHourly = () => ((hourlySliderElement?.querySelector('.hour-card')?.offsetWidth || 90) + 12); btnLeftHourly?.addEventListener("click", () => { if (!hourlySliderElement || hourlySliderElement.children.length === 0) return; const cardScrollWidth = getCardWidthHourly(); currentPositionHourly += cardScrollWidth * 2; if (currentPositionHourly > 0) { currentPositionHourly = 0; } hourlySliderElement.style.transform = `translateX(${currentPositionHourly}px)`; }); btnRightHourly?.addEventListener("click", () => { if (!hourlySliderElement || !hourlySliderElement.parentElement || hourlySliderElement.children.length === 0) return; const cardScrollWidth = getCardWidthHourly(); const containerWidth = hourlySliderElement.parentElement.offsetWidth; const totalCardsScrollWidth = hourlySliderElement.scrollWidth; const maxScroll = -(totalCardsScrollWidth - containerWidth); if (totalCardsScrollWidth <= containerWidth) { currentPositionHourly = 0; } else { currentPositionHourly -= cardScrollWidth * 2; if (currentPositionHourly < maxScroll) { currentPositionHourly = maxScroll; } } hourlySliderElement.style.transform = `translateX(${currentPositionHourly}px)`; });
    let currentPositionDaily = 0; const btnLeftDaily = document.querySelector(".daily-forecast .arrow-left-day"); const btnRightDaily = document.querySelector(".daily-forecast .arrow-right-day"); const getCardWidthDaily = () => ((dailyForecastElement?.querySelector('.daily-card')?.offsetWidth || 180) + 15); btnLeftDaily?.addEventListener("click", () => { if (!dailyForecastElement || dailyForecastElement.children.length === 0) return; const cardScrollWidth = getCardWidthDaily(); currentPositionDaily += cardScrollWidth * 2; if (currentPositionDaily > 0) { currentPositionDaily = 0; } dailyForecastElement.style.transform = `translateX(${currentPositionDaily}px)`; }); btnRightDaily?.addEventListener("click", () => { if (!dailyForecastElement || !dailyForecastElement.parentElement || dailyForecastElement.children.length === 0) return; const cardScrollWidth = getCardWidthDaily(); const containerWidth = dailyForecastElement.parentElement.offsetWidth; const totalCardsScrollWidth = dailyForecastElement.scrollWidth; const maxScroll = -(totalCardsScrollWidth - containerWidth); if (totalCardsScrollWidth <= containerWidth) { currentPositionDaily = 0; } else { currentPositionDaily -= cardScrollWidth * 2; if (currentPositionDaily < maxScroll) { currentPositionDaily = maxScroll; } } dailyForecastElement.style.transform = `translateX(${currentPositionDaily}px)`; });
